...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if (!PermissionMiddleware::check('purchase.create', 'create')) {
            throw new Exception(
                'You do not have permission to create purchase inward.'
            );
        }
        ...
        */
        ...
        foreach ($productIds as $index => $productId) {
            ...
            $manufacturingDate =
                !empty($manufacturingDates[$index])
                    ? $manufacturingDates[$index]
                    : null;

            $expiryDate =
                !empty($expiryDates[$index])
                    ? $expiryDates[$index]
                    : null;
            ...
            */
            ...
            if (
                ...
                throw new Exception(
                    'Expiry date cannot be before manufacturing date for '
        ...
        */
        ...
        if ($status === 'QC_PENDING') {
            ...
            $message =
                'Purchase Inward saved and sent to Quality Check successfully ✅';

        } else {

            $message =
                'Purchase Inward saved as draft successfully ✅';
        }
...
try {
    ...
    $stmt = $db->prepare("
        SELECT
            ...
            COALESCE(
                SUM(pii.received_qty + pii.free_qty),
                0
            ) AS total_qty
        FROM purchase_inwards pi

        LEFT JOIN parties p
            ON p.id = pi.supplier_id

        LEFT JOIN warehouses w
            ON w.id = pi.warehouse_id

        LEFT JOIN purchase_inward_items pii
            ON pii.inward_id = pi.id
...
यह Purchase Entry नहीं है।
यह केवल supplier से माल receive होने की entry है।
...
<td>
<?= e($inward['inward_number']) ?>
</td>

<td>
<?= e($inward['inward_date']) ?>
</td>
...
<td>
<?= (int)$inward['item_count'] ?>
</td>

<td>
<?= number_format(
    (float)$inward['total_qty'],
    3
) ?>
</td>

<td class="status status-<?= e($inward['status']) ?>">
...
<a
    ...
    href="quality_check.php?inward_id=<?= (int)$inward['id'] ?>"
>
Open QC
</a>
...
<?php elseif (
    $inward['status'] === 'QC_APPROVED'
): ?>

<strong>QC Approved</strong>

<?php elseif (
    $inward['status'] === 'QC_PARTIAL'
): ?>

<strong>QC Partial</strong>

<?php elseif (
    $inward['status'] === 'QC_REJECTED'
): ?>

<strong>QC Rejected</strong>

<?php elseif (
    $inward['status'] === 'PURCHASE_CREATED'
): ?>
